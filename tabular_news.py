...
The Project:
A news aggregator that automatically sends customized news, arranged company-wise to
the inbox of the employees to reduce time 
...
def send_email(dataframeslist):
    ...
    recipients =  ["aa729@cornell.edu","369@doonschool.com"]
    message['To'] = ", ".join(recipients)
    message['Subject'] = "Today's News"
    password = "Newstoday@1" # add sender password
    if len(dataframeslist)!=0:
        lista = []
        for df in dataframeslist:
            pd.set_option('display.max_colwidth', -1)
            lista.append(df.to_html())
        lista = " ".join(lista)
        body = lista
    else:
        body = "No relevant news from any company today (so far)!"
...
def read_excel(today_date):
    dataframes_list = []
    companies = ("TCS", "INFY", "HCLT", "WIPRO","TECHM", "PERSISTENT", "L&TTech","eClerx", "Info-edge")
    for company in companies:
        myfile = pd.read_excel('news.xlsx', sheet_name=company)
        col = myfile['Dates']
        for date in col:
            if today_date in date:
                df = myfile.loc[myfile['Dates'] == date]
...
def convert_date(date):
    day = date[0:2]
    month = datetime.datetime.strptime(date, "%d %b").strftime('%B')
    coverted_date = day+ ' '+month
    date_obj = datetime.datetime.strptime(coverted_date, "%d %B")
    return date_obj

def repopulate_excelsheet(dataframeslist):
    """
...
print("Time Elsapsed: %s" % elapsedTime)